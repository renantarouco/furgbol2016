...
void MarcarAreaAdv::executar(Agente *agente)
{
    int id = agente->getId();
...
bool MarcarAreaAdv::verificarTermino(int id)
{
    return false;
}

float MarcarAreaAdv::avaliacaoEscolhaRobo(int id)
{
...
int MarcarAreaAdv::getIdMelhorRobo(vector<int> &_ids)
{
...
bool MarcarAreaAdv::isAtiva()
{
    return false;
}

Tatica *MarcarAreaAdv::getNovaInstancia()
{
    return new MarcarAreaAdv();
}

void MarcarAreaAdv::imprimirNome()
{
    cout << "MarcarAreaAdv" << endl;
}

string MarcarAreaAdv::getNome(){
    return "MarcarAreaAdv";
}

float MarcarAreaAdv::getLimiarAvaliacao()
{
    return 0.0;
}

/** Funcao que calcula o alvo aonde o robo ira se posicionar na defesa */
Ponto MarcarAreaAdv::calcularAlvoPosDefesa(int id){

    /// pegando a area da equipe que usaremos para nos posicionar
    areaAdv = Sistema::modeloMundo.getCampo()->getAreaAdv();

    /// pegando os robos com a mesma tática DefesaCirculo e que se posicionam no mesmo objeto
    vector<Robo*> robosComTatica = Sistema::modeloMundo.getOutrosRobosComTatica(id, "MarcarAreaAdv");
    robosComTatica.push_back(Sistema::modeloMundo.getRoboEq(id));

    /// Calculando o alvo aonde o robô irá se posicionar no poligono
    vector<Ponto> alvos = areaAdv->calcularAlvosPosicionarDefesaFrente(robosComTatica.size());
    if(!alvos.empty()){
